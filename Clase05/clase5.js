...
const spanAlbum = document.querySelector('#cant-albums');
const spanFavoritos = document.querySelector('#cant-favoritos');
const cantidadAlbum = listado.length;
const cantidadFavoritos = listado.filter(function (album) {
    return album.like;
});

spanAlbum.innerText = cantidadAlbum + (cantidadAlbum === 1 ? ' álbum' : ' álbumes');
spanFavoritos.innerText = cantidadFavoritos.length + (cantidadFavoritos.length === 1 ? ' favorito' : ' favoritos')
}
